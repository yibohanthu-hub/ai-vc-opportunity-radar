...
| [36 氪公开 RSS](https://www.36kr.com/rss-center) | 国内创投、产品和科技报道补充 | 使用其公开说明页列出的综合资讯、文章资讯和最新快讯 RSS；只保留标题、摘要、时间和原始链接，再按 AI、创业/产品、地域与排除规则进入待核验层。 |
| [雷峰网公开 RSS](https://www.leiphone.com/us/rss) | 国内 AI、机器人、智能驾驶和投融资报道补充 | 使用其公开列出的人工智能、机器人、智能驾驶、投融资分类 RSS；仍按 AI、创业/产品、地域与排除规则入队，不从标题自动确认主体或融资事实。 |
...
| 活动行：南京、武汉、苏州、西安、重庆、天津、长沙、合肥、厦门、青岛、宁波、东莞、佛山、无锡、济南、郑州、福州、昆明、南昌、沈阳、大连、常州 AI 活动 | 重点产业城市的公开 AI 活动候选 | 与前述目录采用相同的低频、只读方式；共覆盖 28 城，保留名称、日期文本、地点、主办方与原始链接，所有现场信息仍待人工复核。 |

采集遵守低频、只读、公开源原则：每次运行每个来源只请求一次；队列滚动保留近 45 天待核验记录，单类最多保留 120 条项目 / 产品候选和 120 条活动候选。活动行列表页的年份、嘉宾、报名状态与现场机制仍标为待核验；36 氪标题 / 摘要中的融资信息同样不会被直接升级为正式项目事实。
...
新增来源需先通过本地抓取、字段校验和线上 Actions 复测后，才会写入定时任务；公开内容平台（如小红书）仍只允许人工发现，不做登录、绕过限制或批量自动抓取。
