...
def build_pdf():
    ...
    story += [p("我把 V1.7 定义为“可信信息池 + 实时待核验队列”，而不是“AI 投资推荐器”。这既符合投资判断的风险边界，也让产品能先解决高频、可验证的工作。", "body", styles)]
    ...
    decisions = [
        ...
        [p("分层真实数据优先", "card_title", styles), p("60 个国内优先项目（含 35 个早期信号项目）+ 36 个活动作为已整理样本；38 个公开来源（InfoQ 中文、量子位、36 氪、雷峰网公开 RSS / 融资快报与活动行 28 城目录）的新信号先进入近 45 天待核验队列。", "card_body", styles)],
    ...
    process = Table([[p("公开候选", "card_title", styles), p("实体与字段", "card_title", styles), p("去重与可信度", "card_title", styles), p("可解释初筛", "card_title", styles), p("人工反馈", "card_title", styles)], [p("InfoQ / 量子位 / 36 氪 / 雷峰网 / 活动行 28 城", "card_body", styles), p("候选项目 / 产品 / 活动与来源", "card_body", styles), p("标题与来源链接去重；近 45 天保留；缺字段显式保留", "card_body", styles), p("理由、不确定性、验证问题", "card_body", styles), p("收藏、忽略、标签、跟进", "card_body", styles)]], colWidths=[(A4[0] - 36 * mm) / 5] * 5)
